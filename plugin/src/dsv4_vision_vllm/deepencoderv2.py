...
# Derived from FlyCockpit/DeepSeek-V4-Vision-2x-DGX-Sparks at revision
# 7cb20472e0f007a0626bd22ed9f5e22a8825c7e1.
# Detectron2/ViTDet-derived portions retain Apache-2.0 terms; other
# FlyCockpit-derived and repository-local modifications retain MIT terms.
        ...
        class CustomQwen2ModelInner(Qwen2Model):
            ...
            def forward(
                ...
                if inputs_embeds is None:
                    raise ValueError("CustomQwen2ModelInner requires inputs_embeds")
                if token_type_ids is None:
                    raise ValueError("CustomQwen2ModelInner requires token_type_ids")
                ...
                if attention_mask is not None:
                    if attention_mask.dim() != 2:
                        raise ValueError("attention_mask must be a 2-D padding mask")
                    min_dtype = torch.finfo(dtype).min
                    padding_mask = attention_mask[:, None, None, :].to(
                        device=device, dtype=dtype
                    )
                    custom_mask = custom_mask + (1.0 - padding_mask) * min_dtype
