...
RUN DSPARK_LOADER="$(python3 -c 'import inspect; import vllm.v1.worker.gpu.spec_decode.dspark.utils as module; print(inspect.getsourcefile(module))')" \
    && KV_CACHE_MANAGER="$(python3 -c 'import inspect; import vllm.v1.core.kv_cache_manager as module; print(inspect.getsourcefile(module))')" \
    && SCHEDULER="$(python3 -c 'import inspect; import vllm.v1.core.sched.scheduler as module; print(inspect.getsourcefile(module))')" \
